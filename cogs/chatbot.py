...
class sentience(commands.Cog):
    ...
    async def on_message(self, input: discord.Message):
        ...
        if not input.author.bot and sanitized.lower().startswith("dannybot") or sanitized.lower().endswith("dannybot"): # if the random number generator is equal to the sentience ratio and the message is not a command or a message to dannybot
                ...
                model="gpt-3.5-turbo",
                messages=[
                        {"role": "system", "content": "You are Dannybot, created by a man named FDG. Your pronouns are He/Him, and you are 3 years old. Be nonchalant and sarcastic when responding. if you are talking to FDG specifically, be very respectful."}, # Usernames have been provided to you, please do not type 'Dannybot:' when responding.
