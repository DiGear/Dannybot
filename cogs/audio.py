...
class audio(commands.Cog):
    ...
    async def midiflip(self, ctx, *args):
        ...
        os.system(f"fluidsynth -ni {dannybot}\\assets\\SF2\\general.sf2 {midi_output_path} -F {dannybot}\\cache\\midislap_{ctx.message.id}.wav -r 44100")

        ogg_output_path = f'{dannybot}\\cache\\midislap_{ctx.message.id}.ogg'
        os.system(f"ffmpeg-normalize {dannybot}\\cache\\midislap_{ctx.message.id}.wav -o {ogg_output_path} -c:a libopus -b:a 64k --keep-loudness-range-target -f")

        with open(f'{dannybot}\\cache\\midiflip_negative.mid', 'rb') as i, open(ogg_output_path, 'rb') as f:
    ...
    @commands.command(aliases=['nathans'], description="Renders a MIDI file with a random soundfont and sends the resulting audio. You can also choose a specific soundfont from a list of available ones.", brief="Applies a selectable soundfont to a MIDI file")
    async def midislap(self, ctx, *args):
        ...
        await ctx.send("Generating... Use 'd.midislap' on its own to see a list of selectable soundfonts...", delete_after=10)

        midi_output_path = f'{dannybot}\\cache\\midislap_{ctx.message.id}.wav'
        os.system(f"fluidsynth -ni {dannybot}\\assets\\SF2\\{SF2} {dannybot}\\cache\\midislap.mid -F {midi_output_path} -r 44100")

        ogg_output_path = f'{dannybot}\\cache\\midislap_{ctx.message.id}.ogg'
        os.system(f"ffmpeg-normalize {midi_output_path} -o {ogg_output_path} -c:a libopus -b:a 64k -f")
