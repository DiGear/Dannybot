...
class booru(commands.Cog):
    ...
    @commands.hybrid_command(
        name="gelbooru",
        aliases=["gel"],
        description="Browse gelbooru posts using the bot.",
        brief="Browse gelbooru posts using the bot.",
    )
    async def gelbooru(self, ctx: commands.Context, *, tag: str):
        ...
        if not posts:
            return await ctx.send(f"No results found for {tag}.")
        ...
        current_index = 0
        self.last_msg = (
            await self.send_valid_post(ctx, posts, current_index) or self.last_msg
        )
        if self.last_msg:
        ...
        while True:
            try:
                react, user = await self.bot.wait_for(
                    "reaction_add",
                    timeout=60.0,
                    check=self.reaction_check(ctx, self.last_msg),
                )
                ...
                if 0 <= current_index < len(posts):
                    new_embed = await self.send_valid_post(
                        ctx, posts, current_index, True
                    )
    ...
    def reaction_check(self, ctx, msg):
        return (
            lambda react, user: user == ctx.author
            and str(react.emoji) in arrow_emojis
    ...
    async def send_valid_post(self, ctx, posts, idx, return_embed=False):
        while idx < len(posts):
            ...
            if rating in ("explicit") and not (
                ctx.channel.is_nsfw() or isinstance(ctx.channel, discord.DMChannel)
            ):
                idx += 1
            else:
    ...
    async def send_embed(self, ctx, post, return_embed=False):
        ...
        embed = discord.Embed(
            title=f"Gelbooru",
            url=f"https://gelbooru.com/index.php?page=post&s=view&id={id_}",
        )
